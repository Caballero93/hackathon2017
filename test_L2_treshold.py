...
class Solution(object):
    def __init__(self, L2_treshold, P_BATT, L3_TRESHOLD):
        self.p = Process(target=solution.run, args=('log', L2_treshold, P_BATT, L3_TRESHOLD))
    def start(self):
        self.p.start()
...
if __name__ == '__main__':
    ...
    #Testiranje L3_TRESHOLD-a:
    L2_treshold = 6.0
    P_BATT = 6.0
    L3_TRESHOLD = 2.5
    for i in range(26):
        solution_process = Solution(L2_treshold, P_BATT, L3_TRESHOLD)
        solution_process.start()
        framework_process = Framework()
        framework_process.start()
